...
from utils.connection_handler import ai_assistant_write_connection_handler_context


async def log_tokens(user, request_type, tokens_data, url, header="", body="", model=""):
    try:
        async with ai_assistant_write_connection_handler_context() as connection_handler:
